...
func main() {
	...
	for len(open) > 0 {
		...
		if currentPoint.x == endPoint.x && currentPoint.y == endPoint.y && currentPoint.alreadyTravelled > 3 && currentPoint.alreadyTravelled <= 9 {
		...
		for i := -1; i <= 1; i++ {
			for j := -1; j <= 1; j++ {
				...
				// Skip if different direction and less than 4 steps
				if newVector != currentPoint.vector && currentPoint.alreadyTravelled < 3 {
					continue
				}
				// Skip if same direction and more than 10 steps
				if newVector == currentPoint.vector && currentPoint.alreadyTravelled >= 9 {
					continue
				}
				// A*
	...
	// type void struct{}
	// var member void
	// path := make(map[Vector]void)
	// currentPoint := finalPoint
	// for currentPoint.x != 0 || currentPoint.y != 0 {
	// 	path[Vector{x: currentPoint.x, y: currentPoint.y}] = member
	// 	currentPoint = parents[currentPoint]
	// }
	// for i, line := range grid {
	// 	for j := range line {
	// 		char := "."
	// 		if _, exists := path[Vector{x: j, y: i}]; exists {
	// 			char = "#"
	// 		}
	// 		fmt.Print(char)
	// 	}
