...
func main() {
	...
	for k := range histories {
		for i := len(histories[k]) - 1; i > 0; i-- {
			lowerLineFinal := histories[k][i][0]
			upperLineFinal := histories[k][i-1][0]
			histories[k][i-1] = append([]int{upperLineFinal - lowerLineFinal}, histories[k][i-1]...)
		}
		output += histories[k][0][0]
	}
	// Print the result
	fmt.Println(output)
}
