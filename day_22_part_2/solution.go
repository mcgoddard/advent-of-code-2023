...
func main() {
	...
	for id, block := range blocks {
		...
		for i := block.start.x; i <= block.end.x; i++ {
			for j := block.start.y; j <= block.end.y; j++ {
				...
				minZ := int(math.Min(float64(block.start.z), float64(block.end.z)))
	...
	for id := range blocks {
		open := []int{id}
		willFall := make([]int, 0)
		willFall = GetWillFall(open, willFall, supports, supported)
		output += len(willFall) - 1
	}
	// Print the result
	fmt.Println(output)
}
...
func GetWillFall(open []int, willFall []int, supports map[int][]int, supported map[int][]int) []int {
	if len(open) == 0 {
		return willFall
	}
	// Take current block and add to willFall list
	current := open[0]
	open = open[1:]
	willFall = append(willFall, current)
	// For each block this supports check if all it's supporting blocks have fallen
	for _, supportedId := range supports[current] {
		supportsGone := true
		for _, supportingId := range supported[supportedId] {
			if !NumberInSlice(willFall, supportingId) {
				supportsGone = false
			}
		}
		if supportsGone {
			open = append(open, supportedId)
		}
	}
	return GetWillFall(open, willFall, supports, supported)
}
